...
| Key | Type | Mặc định | Mô tả |
|---|---|---|---|
| `n_folds` | int | `5` | Số lượng fold trong stratified k-fold CV. Mọi thuật toán và mọi hyperparameter combo đều được đánh giá trên **cùng bộ folds**, đảm bảo so sánh công bằng. Model cuối cùng được retrain trên **toàn bộ dataset** sau khi chọn xong params tốt nhất. |
| `random_seed` | int \| null | `null` | Seed cố định cho reproducibility (cả fold splitting và model init). `null` = random |
| `ensemble_mode` | string | `"soft_voting"` | Cách kết hợp models: `"soft_voting"` (avg đều) \| `"weighted_voting"` (weighted theo validation metric) \| `"best_model"` (chỉ dùng model tốt nhất) |
...
`n_folds` (global) được dùng cho **cả 2 mode**:

- **Fixed params** (không có `--optimize`): đánh giá `params` cố định trên `n_folds` folds, retrain final model trên full data
- **Optimize** (có `--optimize`): mỗi combo được đánh giá trên cùng `n_folds` folds, combo có mean metric cao nhất được chọn, retrain final model trên full data

CLI flag `--folds N` có thể override `n_folds` trong config cho tất cả thuật toán.

### Metrics

Mỗi fold tính 5 metrics: **F1_score**, **Accuracy**, **Precision**, **Recall**, **MCC**.
Mean của từng metric qua `n_folds` folds được dùng để chọn best combo (khi `--optimize`).
...
| `threshold` | float | `0.5` | Ngưỡng probability cho prediction (dùng chung cho tất cả benchmark modes) |
...
### `adaptive` (object bên trong `benchmark`)

Cấu hình cho adaptive multi-scale benchmark (`run_benchmark_adaptive.py`).

| Key | Type | Mặc định | Mô tả |
|---|---|---|---|
| `default_window` | int | `18` | Kích thước window mặc định (thử trước) |
| `min_window` | int | `11` | Kích thước window nhỏ nhất khi explore |
| `max_window` | int | `25` | Kích thước window lớn nhất khi explore |
| `confidence_margin` | float | `0.15` | Chỉ thử size khác khi \|score − 0.5\| < margin. Nhỏ hơn = ít explore, lớn hơn = nhiều explore |

Khi window score ban đầu nằm trong vùng uncertain [0.35, 0.65], pipeline giữ nguyên start position và thử thay đổi **end position** (mở rộng/thu hẹp) để tìm size mà model confident nhất — có thể là confident positive HOẶC confident negative (công bằng 2 chiều).

CLI flags `--window-size`, `--min-window`, `--max-window`, `--confidence-margin` override các giá trị tương ứng.
