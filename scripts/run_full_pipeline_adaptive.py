#!/usr/bin/env python3
"""Train ensemble + run adaptive multi-scale benchmark in one go."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Full pipeline: train ensemble → adaptive multi-scale benchmark.",
    )
    # ── Training args ──
    ...
    parser.add_argument("--threshold", type=float, default=None, help="Score threshold k.")
    parser.add_argument(
        "--window-size", type=int, default=None, dest="default_window",
        help="Default sliding window size. Default: from config.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Confidence margin. Default: 0.15.",
    )
    parser.add_argument("--name", default="benchmark_adaptive_results", help="Benchmark output filename prefix.")
    parser.add_argument(
        "--classifier",
        action="store_true",
        help=(
    ...
    bench_arts = run_benchmark_adaptive(
        config,
        model_path=ensemble_path,
        input_json=args.bench_input,
    ...
    print("\nFull pipeline (adaptive) completed.")
